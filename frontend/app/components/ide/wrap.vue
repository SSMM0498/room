<template>
  <aside class="flex items-center justify-center mx-2 pb-2 w-1/7 max-w-1/7 h-full">
    ...
    <ide-file-explorer v-else class="panel py-2 px-3 border border-gray-300 rounded-lg bg-gray-100
  ...
  <div class="w-3/7 flex flex-col items-center justify-center h-full mr-2 pb-2">
    ...
    <template v-else>
      <ide-browser :key="url" :defaultUrl="url" :class="`panel ${showTerminal ? 'h-1/2 mb-2' : 'h-full'}`" w />
